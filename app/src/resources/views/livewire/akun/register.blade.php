...
<div x-data="{ showPassword: false, showConfirmPassword: false }"
    ...
    <form wire:submit.prevent="register" novalidate>
        <div class="mb-3">
            ...
            <input id="name" type="text" wire:model.lazy="name" class="w-full" />
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input id="whatsapp_number" type="text" wire:model.lazy="whatsapp_number" class="w-full" />
        ...
        <div class="mb-3 relative">
            ...
            <input type="{{ $showConfirmPassword ? 'text' : 'password' }}" id="password_confirmation"
                wire:model.lazy="password_confirmation" class="w-full pr-10" />
            <button type="button" wire:click="toggleShowConfirmPassword" class="icon-btn"
                aria-label="Toggle password visibility">
            ...
            @if ($password_confirmation && $password_confirmation !== $password)
                <span class="text-red-500">Password tidak sesuai</span>
            @endif
        </div>

        <button type="submit" @disabled(!$name || !$email || !$password || !$password_confirmation || $password !== $password_confirmation)>
            Daftar
        </button>
    </form>
</div>
